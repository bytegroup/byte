...
<table id="report-table" class="display" width="100%" cellspacing="0">
    ...
    <tbody>
    <?php foreach($rows as $quotId => $fields): ?>
        <tr>
            <td><a role="button" href="<?php echo base_url(REPORT_FOLDER.'quotation_details/index/'.$quotId);?>" class="ui-button">Details</a></td>
...
<script language="JavaScript">
    $(document).ready(function(e){
        ...
        $('div.DTTT_container a#excelDownload').click(function(){
            $('form#filter-form')
                .attr('action', baseURL+'quotation_list/get_excel')
                .attr('method', 'post')
                .attr('target', '_blank')
                .submit();
        });
        ...
        $('#filter-button').click(function(){
            filterDataTable(baseURL+'quotation_list/ajax_get_data', filterDT);
        });

        $('#company').change(function(){
            get_dependent_options('company', 'department', baseURL+'quotation_list/ajax_get_department/');
        });
        $('#category').change(function(){
            get_dependent_options('category', 'item', baseURL+'quotation_list/ajax_get_items/');
        });
        ...
        var filterDT = function(data){
            ...
            $.each(data, function(id, val){
                val.unshift('<a role="button" href="'+baseURL+'quotation_details/index/'+id+'" class="ui-button">Details</a>');
