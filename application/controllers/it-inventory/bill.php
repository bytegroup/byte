...
class Bill extends MX_Controller {
    ...
    function index(){
        try{
            ...
            $crud->columns('billTitle','organizationId','billCreated');
            $crud->display_as('billTitle','Title')
                 ->display_as('organizationId','Organization')
                 ->display_as('billCreated','Date');

            //$crud->fields('');
            $crud->unset_texteditor('details', 'description');
           ...
           // $crud->unset_add();
            ...
            if(isset($this->my_session->permissions['canIT-InventoryEdit'])){
                $crud->add_action('Edit', "", IT_MODULE_FOLDER.'bill/edit_Requisition');
            }
            if(isset($this->my_session->permissions['canIT-InventoryDelete'])){
                $crud->add_action('Delete', "", IT_MODULE_FOLDER.'bill/delete_Requisition');
            }
