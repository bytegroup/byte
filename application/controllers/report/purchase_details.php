...
class Purchase_Details extends MX_Controller {
    ...
    function index(){
        try{
            ...
            $output['metadata']= $this->model->get_meta_data(148);
            $output['headers']= $this->model->get_headers();
            $rows= $this->model->get_data(148);
            $output['data']= $rows;
            $output['css'] = "";
            $output['js'] = "";
    ...
    function get_excel(){
        ...
        $this->excel->set_meta_data($sheet, $this->model->get_meta_data(148), 2, 'A', 3, (count($this->model->get_headers())-3));
        $this->excel->set_table($sheet, $this->model->get_headers(), $this->model->get_data(148), $sheet->getHighestRow()+2);
